...
CREATE OR REPLACE PACKAGE BODY WRITE_WAYBILL is
  ...
  vfd constant varchar2(8) := 'YYYYMMDD';
   ...
   /*LOAN DETAILS*/
  function get_DSPLoanID(p_loan in varchar2,
                      p_coi in varchar2,
  ...
  begin
    ...
    where lonmst.lonmst_id=londtl.lonmst_id
      and londtl.delete_useR_id is null
      and lonmst.loan_code= p_loan
      and londtl.si_record_id = coisis.si_record_id
      and coisis.origin_id =stdcmm.origin_id
      and stdcmm.origin_id=p_coi
      and stdcmm.comm_category_code = p_cmmcat
      and stdcmm.commodity_code=p_cmm;
    return true;
  
  exception
      when too_many_rows then
            raise_application_error(-20001,
                              'Too many rows for the loan number: '||p_loan);
   ...
   /*LOAN DETAILS*/
  function get_DSPRepayID(p_loan in varchar2,
                      p_coi in varchar2,
                      p_cmmcat in varchar2,
                      p_cmm in varchar2 ,
                      p_type in varchar2 ) return boolean is
  ...
  /*CHECK_ORG_UNIT*/
  function check_RCPLoanCOI (p_loanID in varchar2,
                             P_londtlID in varchar2,
                             p_coi in varchar2,
                             p_cat in varchar2) return boolean is
    a number;
  begin
    select 1
    into a
    from loan_details londtl
    where londtl.lonmst_id = p_loanID
      and londtl.londtl_id = P_londtlID
      and receiving_origin_id =p_coi
      and comm_category_code = p_cat;
    return true;
  ...
  function check_RCPRepayCOI (p_loanID in varchar2,
                             P_londtlID in varchar2,
                             p_coi in varchar2,
                             p_cat in varchar2) return boolean is
    a number;
...
--> master    
    ...
    dspmst_rec.trailer_plate            := p_vehiclereg;
...
--> details
  ...
  begin
    ...
    */
    ...
    if r_dispatch_masters.tran_type_code  in ('LOAN','REP','SWA1','SWA2') then
         begin
          select *
        ...
        if  r_dispatch_masters.tran_type_code in  ('LOAN','SWA1') then
            if not check_RCPLoanCOI (r_dispatch_details.lonmst_id,r_dispatch_details.londtl_id,
                               p_coi,p_cmmcat) then
                            raise_application_error(-20001,
            ...
            if not check_RCPRepayCOI (r_dispatch_details.lonmst_id,r_dispatch_details.londtl_id,
                               p_coi,p_cmmcat) then
                            raise_application_error(-20001,
    ...
    if p_damagereason is not null then
      begin
        ...
        where loss_damage_causes.name_record_id = b.record_id and
              b.type = 'D' and
              comm_category_code = r_dispatch_details.comm_category_code and
              cause = p_damagereason
              and rownum=1;
      exception
        when no_data_found then
...
--> bulk cargo
      if p_pckcode in ('BK01','BKBE','BKBG','BKBO','BKBT','BULK') then
         p_net := r_dispatch_details.quantity_net - nvl(p_damageunits,0) -
                  nvl(p_lossunits,0);              

         p_gross := r_dispatch_details.quantity_gross - nvl(p_damageunits,0) -
                  nvl(p_lossunits,0);
    ...
    if p_damageunits is not null then
      if p_pckcode in ('BK01','BKBE','BKBG','BKBO','BKBT','BULK') then
         ...
         p_gross := ( p_damageunits * r_dispatch_details.unit_weight_gross) / 1000;
  ...
  -- P_DISPATCHDT
    ...
    if dspmst_rec.origin_type = 2 and not check_org_unit(dspmst_rec.origin_code) then
    ...
    if dspmst_rec.lti_id is not null and get_lti_masters(dspmst_rec.lti_id).code is null then
...
-->         DSP3 for Disposal Sale;
    ...
    if dspmst_rec.tran_type_code not in ('DEL', 'WIT','DSP1','DSP2','DSP3',
        'LOAN','DST','AIR','REP','SWA1','SWA2','SHUN') then
...
--> Verify loan details
    if dspmst_rec.tran_type_code IN ('LOAN','SWA1') and
       not get_DSPLoanID(p_loan,
                      ...
                      dspdtl_rec.commodity_code,'D') then
    ...
    if get_person(dspmst_rec.person_code,dspmst_rec.person_ouc).code is null then
    ...
    exception
      when dup_val_on_index then
        ...
        where code = dspdtl_rec.code and
              ...
              allocation_destination_code =dspdtl_rec.allocation_destination_code and
              quality = dspdtl_rec.quality;
    END;
